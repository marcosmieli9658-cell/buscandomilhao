...
export class DeterministicConversationEngine implements ConversationEngine {
  ...
  async decideNextAction(leadId: number): Promise<ConversationDecision> {
    ...
    if (/whats|interessad|quero/i.test(lastInbound)) {
      const business = getBusinessConfig();
      return { intention: "wants_whatsapp", action: "send_to_whatsapp", message: `Perfeito. Você pode falar diretamente com a ${business.companyName} por aqui: ${business.whatsappLink}`, reasoningSummary: "Qualified handoff", claimIndices: [], followUpHours: null };
    }
