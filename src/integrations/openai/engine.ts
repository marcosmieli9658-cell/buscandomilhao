...
export class OpenAiConversationEngine implements ConversationEngine {
  ...
  async generateFirstContact(leadId: number): Promise<{ message: string; variantReason: string }> {
    ...
    const decision = await this.callStructured(
      ...
      "first_contact",
      getEnv().OPENAI_MODEL,
      `Você escreve uma primeira DM curta, pessoal, respeitosa e verdadeira em PT-BR. Baseie a abertura somente no perfil público fornecido. Não finja ser cliente. Não prometa resultado. Não fale de preço. Use no máximo uma afirmação comercial e somente copiando o sentido de VERIFIED_CLAIMS. Se não houver contexto suficiente, escale. VERIFIED_CLAIMS indexadas: ${business.verifiedClaims.map((claim, index) => `${index}: ${claim}`).join(" | ")}. UNVERIFIED_CLAIMS proibidas: ${business.unverifiedClaims.join(" | ")}.`,
      JSON.stringify({ lead: { handle: lead.instagramHandle, displayName: lead.displayName, bio: lead.bio, category: lead.category, location: lead.location, segment: lead.segment }, companyPitch: business.oneLinePitch }),
    );
